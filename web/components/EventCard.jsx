...
export default function EventCard({ event, className = "" }) {
  ...
  return (
    ...
    <div className="absolute bottom-0 left-0 right-0 p-4">
        {orgName && (
        <p className="text-white/80 text-[12px] tracking-wider font-semibold mb-1 truncate">by {orgName}</p>
        )}
        <p className="text-white font-display text-[24px] leading-[100%] uppercase tracking-[0.2px] line-clamp-2">
        {name}
        </p>
    </div>
    </Link>
