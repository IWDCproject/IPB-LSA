...
const S = {
  ...
  label: {
    ...JK, fontWeight: 700, fontSize: 15, lineHeight: 1.3,
    whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis",
  },
  meta: {
    ...JK, fontSize: 11, fontWeight: 600, opacity: 0.6,
    whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis",
  },
  liveBadge: {
    ...BB,
  ...
  participantName: {
    ...JK, fontWeight: 700, fontSize: 15,
    whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis",
  },
  participantInst: {
    ...JK, fontSize: 12, fontWeight: 600, opacity: 0.6,
  ...
  pill: {
    ...JK, background: "rgba(255,255,255,0.15)", borderRadius: 4,
    ...
    <div style={{ textAlign: "center" }}>
      ...
      {setLog.length > 0 && (
        <div style={{ display: "flex", gap: 6, justifyContent: "center", marginTop: 8, flexWrap: "wrap" }}>
          {setLog.map((s, i) => <span key={i} style={S.pill}>{s.label ?? `Set ${i + 1}`}: {s.home}–{s.away}</span>)}
        </div>
      )}
    </div>
  );
...
function FinishTime({ live }) {
  ...
  if (!log.length) return <div style={{ ...JK, fontSize: 12, opacity: 0.4, textAlign: "center" }}>Waiting for results…</div>;
  return (
...
function ManualPick({ live, match }) {
  const winner   = live?.winner   ?? null;
  const rankings = live?.rankings ?? [];
  const count    = match?.participant_ids?.length ?? null;
  ...
  if (winner) return <div style={{ ...JK, textAlign: "center", fontWeight: 700, fontSize: 15 }}>🏆 {winner}</div>;
  return <div style={{ ...JK, fontSize: 12, fontWeight: 600, opacity: 0.5, textAlign: "center" }}>{count !== null ? `${count} participants` : "Waiting…"}</div>;
}

const OPEN_SHOW = 4;
function OpenParticipants({ match }) {
  const ids   = match?.participant_ids ?? [];
  const shown = ids.slice(0, OPEN_SHOW);
  const rest  = ids.length - shown.length;
  if (!ids.length) return null;
  return (
    <div style={{ padding: "12px 18px 0", display: "flex", flexDirection: "column", gap: 3 }}>
      {shown.map((id, i) => (
        <div key={id} style={{ ...JK, fontSize: 13, fontWeight: 600, display: "flex", alignItems: "center", gap: 6 }}>
          <span style={{ opacity: 0.5, fontSize: 11, fontWeight: 700, minWidth: 16 }}>{i + 1}</span>
          <span style={{ opacity: 0.9 }}>{id}</span>
        </div>
      ))}
      {rest > 0 && <div style={{ ...JK, fontSize: 12, fontWeight: 600, opacity: 0.5, marginTop: 2 }}>+{rest} more competing…</div>}
    </div>
  );
}
...
export function MatchCard({ match }) {
  ...
  return (
    <div style={{ ...S.card, background: event?.card_image_url ? undefined : "rgba(255,255,255,0.08)" }}>
      {event?.card_image_url && <div style={{ ...S.cardBg, backgroundImage: `url(${event.card_image_url})` }} />}
      ...
      <div style={S.cardInner}>
        ...
        <div style={{ marginTop: "auto", padding: "8px 18px 16px" }}>
          <div style={S.meta}>{event?.name}{match.venue ? ` · ${match.venue}` : ""}</div>
        </div>
