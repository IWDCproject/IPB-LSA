...
function ringGradientFor(isActive, isHovered) {
  if (isActive)  return "linear-gradient(to bottom, rgba(234,179,8,0.9), rgba(234,179,8,0.9))";
  if (isHovered) return `linear-gradient(to bottom, ${YEL}, ${WHT})`;
  return "linear-gradient(to bottom, rgba(255,255,255,0.45), rgba(255,255,255,0.45))";
}
...
function CardNotch({ color, textColor, label }) {
  ...
  return (
    <div style={{
      ...
      // max() biar notch nggak keclip di kartu mobile yang sempit
      left: "max(25%, 50px)",
      zIndex: 20,
...
function PlaceholderCard() {
  return (
    <div className="w-full h-full flex flex-col items-center justify-center p-4"
         style={{ background: "rgba(255,255,255,0.0)", backdropFilter: "blur(4px)", WebkitBackdropFilter: "blur(4px)" }}>
      <div style={{ width: "100%", height: 18 }} />
      <div style={{
        position: "absolute", inset: 0,
        backgroundImage: "url(/Batik_Pattern_white.svg)",
        backgroundSize: "cover", backgroundRepeat: "no-repeat", backgroundPosition: "center",
        opacity: 0.3, pointerEvents: "none", zIndex: 0, filter: "blur(1.5px)",
      }} />
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none"
           stroke="rgba(255,255,255,1)" filter="opacity(0.4)" strokeWidth="1.5"
...
function EventCardSlot({ ev, isActive, isHovered, showBorder = false, children }) {
  return (
    <>
      <div style={{
...
function CardItem({ ev, idx, isActive, isHovered, onSelect, onHover, className, style }) {
  return (
    <div
      onClick={ev ? (e) => { e.preventDefault(); onSelect(idx); } : undefined}
      onMouseEnter={() => ev && !isActive && onHover(idx)}
      onMouseLeave={() => onHover(null)}
      className={className ?? `flex-1 min-w-0 relative ${ev ? "cursor-pointer" : "cursor-default"}`}
      style={{
        borderRadius: "11px",
        padding: ev ? 2 : 0,
        background: ev ? ringGradientFor(isActive, isHovered) : "transparent",
        boxShadow: isActive
          ? "0 4px 16px rgba(0,0,0,0.7), 0 0 2px rgba(240,165,0,0.4)"
          : "0 4px 16px rgba(0,0,0,0.7)",
        transition: "background 0.2s ease",
        overflow: "visible", // notch perlu keliatan di bawah kartu
        ...style,
      }}
    >
      <EventCardSlot ev={ev} isActive={isActive} isHovered={isHovered} showBorder={!ev}>
...
export default function HeroSection({ paused = false, events: rawEvents = [] }) {
  ...
  const heroManifest = useMemo(() => {
    ...
    return EVENTS.map(ev => ({
      url: ev.image_url, type: "hero",
      width: 1920 * dpr, height: 1080 * dpr,
    ...
    <section ref={sectionRef} className="relative w-full h-full flex flex-col overflow-hidden bg-black" style={{ "--s": scale }}>
      ...
      <div className="absolute inset-0 z-0">
        {EVENTS.map((ev, idx) => (
          <div key={ev.id} className="absolute inset-0" style={{ opacity: idx === activeIdx ? 1 : 0, transition: paused ? "none" : "opacity 0.8s ease" }}>
            <Image src={ev.image_url} alt="" fill className="object-cover" aria-hidden="true" />
      ...
      {isMobile ? (
        <div className="absolute z-10" style={{ bottom: "60px", left: 0, right: 0 }}>
          ...
          <div
            ...
            style={{
              ...
              paddingLeft: "24px", paddingBottom: "20px",
          ...
          >
            {EVENTS.map((ev, idx) => (
              <CardItem
                ...
                isHovered={idx !== activeIdx && hoveredIdx === idx}
                onSelect={select}
        ...
        <div
          ...
          style={{
            bottom: "40px",
            left: "clamp(40px, 8.33vw, 160px)",
            right: "clamp(40px, 8.33vw, 160px)",
            paddingBottom: "clamp(32px, 3.125vw, 60px)",
          }}
        >
          <p style={{
            fontFamily: "'Plus Jakarta Sans', sans-serif",
            fontSize: "calc(14px * var(--s))", fontWeight: 700,
            letterSpacing: "0.15em", textTransform: "uppercase",
            color: "#fff", marginBottom: "calc(20px * var(--s))",
            ...(mounted ? introStyle(480) : { opacity: 0 }),
          }}>
            {">>>"} Featured Events
          </p>
            ...
            {mounted && Array.from({ length: desktopSlots }, (_, idx) => {
              ...
              return (
                <CardItem
                  ...
                  ev={ev} idx={idx}
                  isActive={ev && idx === activeIdx}
                  isHovered={ev && idx !== activeIdx && hoveredIdx === idx}
                  onSelect={select}
                  onHover={setHoveredIdx}
                  style={{ height: "calc(240px * var(--s))" }}
                />
              );
            })}
          </div>
