...
// (These are no longer used by FormatBuilderPage but kept to avoid import errors
//  in case other files reference them.)

export function MatchTypePanel() {
  const { matchType, setMatchType } = useFormatBuilder()
  return (
    <div className="space-y-2">
      {MATCH_TYPE_OPTIONS.map((opt) => (
        ...
        >
          ...
          <p className={['text-xs mt-0', matchType === opt.value ? 'text-zinc-300' : 'text-zinc-500'].join(' ')}>
            {opt.description}
          </p>
        </button>
      ))}
...
export function ScoringEnginePanel() {
  const { matchType, engine, setEngine } = useFormatBuilder()
  const availableEngines = ENGINE_OPTIONS.filter((e) => e.matchTypes.includes(matchType))
  const EngineConfig = ENGINE_CONFIG_COMPONENT[engine.type]
  return (
    <div className="space-y-4">
      <div className="space-y-1.5">
        <Label>Scoring Engine</Label>
        <Select value={engine.type} onValueChange={(val) => setEngine({ type: val as EngineType })}>
...
export function AddOnsPanel() {
  const { addOns, setAddOn } = useFormatBuilder()
  return (
    <div className="space-y-4">
      <div className="rounded-sm border border-zinc-200 p-4 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-semibold">Timer</p>
            <p className="text-xs text-zinc-500 mt-0">Countdown, stopwatch, atau deadline</p>
          </div>
          <input
            type="checkbox"
            className="h-4 w-4 rounded border-zinc-300"
            checked={addOns.timer.enabled}
            onChange={(e) => setAddOn('timer', { enabled: e.target.checked })}
          />
        </div>
        {addOns.timer.enabled && (
          <div className="space-y-3 pt-1">
            <div className="space-y-1.5">
              ...
              >
                ...
                <SelectContent>
                  {(Object.keys(TIMER_MODE_LABEL) as TimerMode[]).map((m) => (
                    <SelectItem key={m} value={m}>{TIMER_MODE_LABEL[m]}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
      ...
      <div className="rounded-sm border border-zinc-200 p-4">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-semibold">Catatan Operator</p>
            <p className="text-xs text-zinc-500 mt-0">Kolom catatan bebas saat pertandingan</p>
          </div>
          <input
            type="checkbox"
            className="h-4 w-4 rounded border-zinc-300"
            checked={addOns.notes.enabled}
            onChange={(e) => setAddOn('notes', { enabled: e.target.checked })}
          />
        </div>
      </div>
    </div>
